...
export const PATCH = asyncHandler(async (req: NextRequest, ctx: Ctx) => {
  ...
  await writeAudit(admin, { action: "UPDATE_CATEGORY", entityType: "category", entityId: data.id });
...
export const DELETE = asyncHandler(async (_req: NextRequest, ctx: Ctx) => {
  ...
  const { data, error } = await supabaseAdmin()
    ...
    .single();
  if (error || !data) throw new NotFoundError("Category not found");
  await writeAudit(admin, { action: "DEACTIVATE_CATEGORY", entityType: "category", entityId: data.id });
  revalidatePath("/admin/catalog");
